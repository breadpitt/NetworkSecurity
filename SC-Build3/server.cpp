...
void StartChat(int client) {
    ...
    while (true) {
        ...
        if (ReceiveAndEnvelopeDecryptMessage(client, self_pubpri_key, buffer, buffer_size-1) == FAILURE) {
        ...
        if (EnvelopeEncryptAndSendMessage(client, peer_pub_key, (byte *)msg.c_str(), msg.length()) == FAILURE) {
...
int main() {
    ...
    // read RSA keys
    ReadRSAKeys("server_keys/client_pub.pem","server_keys/server_pubpri.pem", &peer_pub_key, &self_pubpri_key);
