...
void StartChat(int server) {
    ...
    while (true) {
        ...
        if (EnvelopeEncryptAndSendMessage(server, peer_pub_key, (byte *)msg.c_str(), msg.length()) == FAILURE) {
        ...
        if (ReceiveAndEnvelopeDecryptMessage(server, self_pubpri_key, buffer, buffer_size-1) == FAILURE) {
...
int main()
{
    ...
    // read RSA keys
    ReadRSAKeys("client_keys/server_pub.pem","client_keys/client_pubpri.pem", &peer_pub_key, &self_pubpri_key);
